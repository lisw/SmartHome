...
do
  ...
  for k,v in pairs(codes) do t[#t+1] = pre..v.."="..k..";\n" end
  for k,v in pairs(types) do t[#t+1] = pre..v.."="..k..";\n" end  
  pre = pre.."OPEN_"
  for k,v in pairs(opens) do t[#t+1] = pre..k.."="..v..";\n" end  
end
...
assert(ok and sql, 'sqlite3 library not found');
...
return function(stmt_or_value <opt_i>)
  ...
  if t == sql.SQLITE_INTEGER or t == sql.SQLITE_FLOAT then
...
local open_modes = {
  ...
  rwc = sql.SQLITE_OPEN_READWRITE + sql.SQLITE_OPEN_CREATE
}
...
function conn_mt:prepare(stmtstr) T_open(self)
  ...
  local code = sql.sqlite3_prepare_v2(self._ptr, stmtstr, #stmtstr, aptr, tail)
  T_okcode(self._ptr, code)
  local stmt = stmt_ct(aptr[0], false, self._ptr, code)
  connstmt[self][stmt] = true
  return stmt,ffi.string(tail[0])
end

-- Connection exec, query, __call --------------------------------------------
function conn_mt:exec(commands, func) T_open(self)
	local code;
	if func then
		local cb = ffi.cast("int (*)(void*,int,char**,char**)", func);
		code = sql.sqlite3_exec(self._ptr, commands, cb, nil, nil);
		cb:free();
	else
		code = sql.sqlite3_exec(self._ptr, commands, nil, nil, nil);
	end;
	T_okcode(self._ptr, code);
end
...
function conn_mt:__call(commands) T_open(self)
	local n;
	local function show(ctx, columns, values, names)
		if n ~= names then
			n = names;
			for i=0,columns-1 do io.write(ffi.string(names[i]), '\t'); end;
			io.write('\n');
		end;
		for i=0,columns-1 do
			local v = values[i];
			v = v == NULL and "NULL" or ffi.string(v);
			io.write(v, '\t');
		end;
		io.write('\n');
		return 0;
	end;
	self:exec(commands, show);
end;

function conn_mt:query(commands, mode, maxrecords) T_open(self)
  local stmt = self:prepare(commands);
  local rows, cols = stmt:step(mode, maxrecords);
  stmt:close();
  return rows, cols;
end

function conn_mt:changes() T_open(self)
  return sql.sqlite3_changes(self._ptr)
end
...
-- mode: "n": numerical indices(default); "a": alphanumerical indices
stmt_step = function(self, mode, maxrecords)
  ...
  if self._code == sql.SQLITE_ROW then
    ncol = sql.sqlite3_column_count(self._ptr);
    cols = {};
    for i=1,ncol do
      cols[i] = ffi.string(sql.sqlite3_column_name(self._ptr, i-1));
    end;
  elseif self._code == sql.SQLITE_DONE then -- Have finished now.
    return sql.sqlite3_changes(self._conn);
  ...
  while true do
    local row = {};
    for i=1,ncol do
      row[(mode=="a") and cols[i] or i] = get_column(self._ptr, i-1);
    end;
    rows[1+#rows] = row;
    maxrecords = maxrecords - 1;
    if maxrecords <= 0 then break; end;
    self._code = sql.sqlite3_step(self._ptr);
    if self._code ~= sql.SQLITE_ROW then break; end;
  end
  return rows, cols;
end
stmt_mt.step = stmt_step;
...
return {
  open = open,
  blob = blob,
  version = ffi.string(sql.sqlite3_libversion()),
}
